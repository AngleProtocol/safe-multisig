...
contract Utils is Test {
    ...
    function _chainToForkAndSafe(uint256 chainId) internal view returns (uint256, address) {
        return
            chainId == CHAIN_ETHEREUM ? (ethereumFork, address(safeEthereum)) : chainId == CHAIN_OPTIMISM
                ? (optimismFork, address(safeOptimism))
                : chainId == CHAIN_POLYGON
                ? (polygonFork, address(safePolygon))
                : chainId == CHAIN_ARBITRUM
                ? (arbitrumFork, address(safeArbitrum))
                : (avalancheFork, address(safeAvalanche));
    }
